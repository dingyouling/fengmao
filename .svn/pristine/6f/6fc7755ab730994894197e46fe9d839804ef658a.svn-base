	...
	<div class="paytrains">
		<div>
	      ...
	      <h1 class="tc f36">立即支付{{$route.query.payBusiType === 'payTrainsTicketChange' ? '(改签)' : ''}}</h1>
            ...
            <span class="f28 c99 fr">{{payInfo.trainNo}} {{payInfo.fromStation}}-{{payInfo.toStation}}</span>
					</div>
					<div class="clearfix">
						<span class="f30 c66 fl">席别：</span>
						<span class="f28 cl fr">{{$route.query.ticketId ? payInfo.seatClassStr : payInfo.ticketList[0].seatClassStr}}</span>
					</div>
					<div class="clearfix">
