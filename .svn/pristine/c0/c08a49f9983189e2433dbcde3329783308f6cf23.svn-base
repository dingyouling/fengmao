<template>
  <div class="zsh pd">
    <div class="dyl_gas_nums clearfix">
      ...
      <router-link :to="'/choosecard?cardType=1&subCardType=2&returnUrl='+$route.path" class="dyl_gas_recharge_chioce fr"></router-link>
    </div>
    <!--库存-->
    ...
    <div class="dyl_row clearfix" id="gas_list">
      <div class="dyl_gas_c33 tc fl bz gas flag" @click="chooseIndex=index" v-for="(val, index) in zshInfo" :key="index" :class="{active:chooseIndex==index}">
        ...
        <div v-show="val.isAnyAmount === 2" @click="focus" class="focus dc" :class="{active: freeFocus}">
          ...
          <p class="f22 co">售价:{{(freeAmount * val.discountPrice).toFixed(2)}}元</p>
        </div>
      </div>
    </div>
    <!-- 索要发票 -->
...
export default {
  ...
  methods: {
    ...
    subInfo() {
      ...
      subInfo['prodCode'] = this.zshInfo[this.chooseIndex].prodCode
      if (this.zshInfo[this.chooseIndex].isAnyAmount === 2) {
        if (this.freeAmount >= this.zshInfo[this.chooseIndex].numLimit.split('-')[0] && (this.zshInfo[this.chooseIndex].numLimit.split('-')[1] - this.freeAmount >= 0)) {
          subInfo['prodNum'] = this.freeAmount
          subInfo['discountPrice'] = (this.freeAmount * this.zshInfo[this.chooseIndex].discountPrice).toFixed(2)
          subInfo['origPrice'] = this.freeAmount
        } else {
          alert('充值范围' + this.zshInfo[this.chooseIndex].numLimit)
          return false
        }
      } else if (this.zshInfo[this.chooseIndex].isAnyAmount === 1) {
        subInfo['prodNum'] = 0
        subInfo['discountPrice'] = this.zshInfo[this.chooseIndex].discountPrice
        subInfo['origPrice'] = this.zshInfo[this.chooseIndex].origPrice
      }
      // 号箱选择
